...
class RNN_Model(object):
    ...
    def __init__(self,config,is_training=True):
        ...
        #f=open('save.pkl','rb')
        #arr=pickle.load(f)
        #print('loaded pkl array:',arr)
        #lstm_cell = tf.contrib.rnn.LSTMCell(hidden_neural_size,forget_bias=0.0,state_is_tuple=True,reuse=tf.get_variable_scope().reuse,initializer=tf.constant_initializer(arr))
        lstm_cell=tf.contrib.rnn.LSTMCell(hidden_neural_size,forget_bias=0.0,state_is_tuple=True,reuse=tf.get_variable_scope().reuse)
        if self.keep_prob<1:
